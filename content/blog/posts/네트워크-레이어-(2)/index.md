...
같은 AS에 있는 라우터들은 동일한 intra domain protocol을 사용한다.
...
두 BGP 라우터가 세션을 구성하여 반영구적으로 계속 BGP 메세지를 주고 받는다. 새로운 라우터가 서브넷에 가입되면 eBGP를 통해 다른 AS에 전파되고, 그 AS에서는 iBGP를 통해 AS 내부의 라우터에 전파된다.
